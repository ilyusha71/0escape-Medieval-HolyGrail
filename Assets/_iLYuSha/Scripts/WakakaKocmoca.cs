...
public class WakakaKocmoca : MonoBehaviour
{
    ...
    /* Arduino Setting */
    public string serialCOMPort = "COM3";
    ...
    void Awake()
    {
        ...
        story =
           ...
           "鮑斯:教士還沒來得及阻止，<color=#191970>珀西瓦里</color>竟也追了進去，\n我也立刻跟了上去。\n　　　　　　　　　　　- 鮑斯 -@" +
    ...
    {
        arduinoSerialPort = new SerialPort(serialCOMPort, baudRate);
        try
        {
    ...
    private void GetArduino()
    {
        while (myThread.IsAlive && !cancel)
        {
            if (arduinoSerialPort.IsOpen)
            {
                try
                {
                    command = arduinoSerialPort.ReadLine(); //Read the information            
                    //Debug.Log(command);
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.F10))
            Observatory.SetActive(!Observatory.activeSelf);
        ...
        if (command == "Locked" || Input.GetKeyDown(KeyCode.L))
        {
            locked = true;
            Lock.SetActive(true);
        }
        ...
        else if (action == Action.Story)
        {
            ...
            {
                if (command == "Release" || Input.GetKeyDown(KeyCode.D))
                {
                    msgOracle = listOracle[1];
                    Oracle();
                }
                else if (command == "Refuse" || Input.GetKeyDown(KeyCode.F))
                {
    ...
    void SayWhat()
    {
        textStory.color = new Color(textStory.color.r, textStory.color.g, textStory.color.b, 0);
        sequence = DOTween.Sequence();
        Tweener alpha1 = DOTween.To(() => textStory.color, x => textStory.color = x, new Color(textStory.color.r, textStory.color.g, textStory.color.b, 1), 2.0f);
        Tweener alpha2 = DOTween.To(() => textStory.color, x => textStory.color = x, new Color(textStory.color.r, textStory.color.g, textStory.color.b, 0), 0.6f);
